...
const useAppBridgeAuth = ({
  ...
  const [status, setStatus] = useState<"init" | "authenticating" | "authenticated" | "error">(
    "init",
  );
    ...
    if (event.data.action === "validated") {
      ...
      try {
        ...
        if (response.ok) {
          ...
          sessionStorage.setItem(KEY_VALIDATED_PAYLOAD, JSON.stringify(response.result));
          setStatus("authenticated");
          setError(undefined);
          await authenticated();
        } else {
...
const useOAuth = ({ type }: { type: PluginType }) => {
  const [status, setStatus] = useState<"init" | "authenticating" | "authenticated">("init");

  const init = async () => {
    setStatus("authenticating");

    const initOAuth = new URLSearchParams(location.search).get("init_oauth") === "true";
...
export const useAppBridge = ({ type, oauth }: { type: PluginType; oauth: boolean }) => {
